...
type LoginFuncionarioResponse = {
  token: string;
  user: { id: string; nome: string; cpf: string; cargo: string; role: string };
};
...
export default function LoginFuncionario() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const res = await api.post<LoginFuncionarioResponse>(
        "/auth/login/funcionario",
        { cpf: cpfLimpo, senha },
      );
      auth.setToken(res.token);
      localStorage.setItem("clockup.user", JSON.stringify(res.user));
      navigate("/home");
    } catch (e) {
      const msg =
